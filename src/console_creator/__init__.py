...
class Console:
	...
	def add_command(self, *command, **command_kwargs):
		"""
		Adds a command to the console.
		The most important commands should be added first.
		:param command: The command to add. Should be an instance of Command. Otherwise, the command will be created in place.
		"""
		...
		# If it is not a command, then we create a command in place
		if not isinstance(command[0], Command):
			command = Command(*command, **command_kwargs)
		else:
			command = command[0]
...
class Command:
	...
	def __init__(self, name:(str, list, tuple), function_to_call, description:str=None,
		...
		"""
		:param name: A string or a tuple/list of strings that defines the name(s) of the command. Should not contain any space.
